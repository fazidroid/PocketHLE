...
impl PocketLauncher {
    ...
    fn ui_game_card(&mut self, ui: &mut egui::Ui, game: &GameEntry, width: f32) {
        let card_size = Vec2::new(width, 224.0);
        let (card_rect, card_response) = ui.allocate_exact_size(card_size, Sense::click());
        ...
        let menu_rect = Rect::from_min_size(
            Pos2::new(card_rect.right() - 36.0, card_rect.top() + 4.0),
            Vec2::new(32.0, 32.0),
        );
        let mut menu_ui = ui.child_ui(menu_rect, egui::Layout::right_to_left(egui::Align::Center));
        menu_ui.menu_button(RichText::new("⋮").size(24.0), |ui| {
        ...
        let icon_rect = Rect::from_min_size(
            ...
            Vec2::new(width, 152.0),
        );
        let icon_size = (width - 24.0).clamp(56.0, 112.0);
        ...
        let label_rect = Rect::from_min_size(
            Pos2::new(card_rect.left(), card_rect.top() + 156.0),
            Vec2::new(width, 68.0),
        );
        ui.painter()
            .rect_filled(label_rect, 0.0, Color32::from_rgb(28, 29, 32));
        let text_rect = Rect::from_min_size(
            Pos2::new(label_rect.left() + 12.0, label_rect.top() + 9.0),
            Vec2::new(width - 24.0, 50.0),
        );
        let mut text_ui = ui.child_ui(text_rect, egui::Layout::top_down(egui::Align::Min));
        text_ui.set_width(width - 24.0);
        text_ui.set_height(50.0);
        let publisher = game
            .provider
            .as_deref()
            ...
            .unwrap_or("Unknown");
        text_ui.add(
            egui::Label::new(RichText::new(&game.display_name).strong().size(18.0)).truncate(true),
        );
        text_ui.add(
            egui::Label::new(
